  ...
  return (
    <header className={`bg-custom-headerColorBG fixed w-full py-4 px-10 ${isScrolled ? ' shadow-md bg-cyan-700' : 'bg-cyan-700 bg-opacity-0'}`} style={{ transition: 'background-color 0.3s' }}>
      <nav className="container mx-auto flex flex-col lg:flex-row lg:justify-between items-center">
        ...
        <div>
          <a href="/Project" className={`text-[${textChanger()}] hover:text-black mr-4 ${transition()}`}>Projects</a>
          <a href="/Resume" className={`text-[${textChanger()}] hover:text-black-300 mr-4 ${transition()}`}>Resume</a>
          <a href="/Contact" className={`text-[${textChanger()}] hover:text-black-300 ${transition()}`}>Contact</a>
        </div>
      </nav>
    </header>
  )
